...
const rteVerilogSerialCompositionTemplate = `
...
//Warning: This is experimental series composition code.
...
module series_sim_FSM ({{range $index, $var := $block.InputVars}}
	...
	input wire clk,
	{{range $polI, $pol := $block.Policies}}
	output wire clk_input_{{$pol.Name}},
	output wire clk_output_{{$pol.Name}},
	{{end}}
	output wire clk_transition,
	output wire {{getFSMStateWidthDeclareString $block}} state_out
);
	reg {{getFSMStateWidthDeclareString $block}} c_state = 0;

	{{range $polI, $pol := $block.Policies}}
	reg c_in_{{$pol.Name}} = 0;
	reg c_out_{{$pol.Name}} = 0;
	{{end}}
	reg c_trans = 0;
	{{range $index, $var := $block.InputVars}}
	reg {{$var.Name}}_enf = 0;
	...
	begin{{range $index, $var := $block.InputVars}}
		...
		if (c_state == {{getLastFSMState $block}}) begin
			c_state = 0;
		end
		else begin
			c_state = c_state + 1;
	...
	{{range $polI, $pol := $block.Policies}}
	assign clk_input_{{$pol.Name}} = c_in_{{$pol.Name}};
	assign clk_output_{{$pol.Name}} = c_out_{{$pol.Name}};
	{{end}}
	assign clk_transition = c_trans;

endmodule

module series_F_{{$block.Name}}(
	...
	{{end}}
	output wire clk_transition;
	output wire {{getFSMStateWidthDeclareString $block}} fsm_state;
	{{range $index, $var := $block.InputVars}}
	...
	output wire {{getVerilogWidthArray (getMaxRecoveryReference $pol)}} {{$block.Name}}_policy_{{$pol.Name}}_output_recovery_ref;
	{{end}}
	series_sim_FSM instance_series_sim_FSM({{range $index, $var := $block.InputVars}}
		...
		.clk(clk),
		{{range $polI, $pol := $block.Policies}}
		.clk_input_{{$pol.Name}}(clk_input_{{$pol.Name}}),
		.clk_output_{{$pol.Name}}(clk_output_{{$pol.Name}}),
		{{end}}
	...
	F_LUT_Output_Edit instance_LUT_Input_Edit(
		...
		{{end}}
		.clk(clk_input_{{getLastPolicy $block}})
	);
	{{range $polI, $pol := $block.Policies}}
	...
	F_LUT_Output_Edit instance_LUT_Output_Edit(
		...
		.{{$block.Name}}_policy_{{$pol.Name}}_output_recovery_ref({{$block.Name}}_policy_{{$pol.Name}}_output_recovery_ref),
		{{end}}
		.clk(clk_output_{{getLastPolicy $block}})
	);
	{{range $polI, $pol := $block.Policies}}
...
var verilogSerialCompositionTemplateFuncMap = template.FuncMap{
...
var verilogSerialCompositionTemplates = template.Must(template.New("").Funcs(verilogSerialCompositionTemplateFuncMap).Parse(rteVerilogSerialCompositionTemplate))
